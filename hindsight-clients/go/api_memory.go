...
type ApiRetrieveSemanticCandidatesRequest struct {
	ctx context.Context
	ApiService *MemoryAPIService
	bankId string
	semanticCandidatesRequest *SemanticCandidatesRequest
	authorization *string
}

func (r ApiRetrieveSemanticCandidatesRequest) SemanticCandidatesRequest(semanticCandidatesRequest SemanticCandidatesRequest) ApiRetrieveSemanticCandidatesRequest {
	r.semanticCandidatesRequest = &semanticCandidatesRequest
	return r
}

func (r ApiRetrieveSemanticCandidatesRequest) Authorization(authorization string) ApiRetrieveSemanticCandidatesRequest {
	r.authorization = &authorization
	return r
}

func (r ApiRetrieveSemanticCandidatesRequest) Execute() (*SemanticCandidatesResponse, *http.Response, error) {
	return r.ApiService.RetrieveSemanticCandidatesExecute(r)
}

/*
RetrieveSemanticCandidates Retrieve semantic candidates
...
Return a bounded, score-ordered candidate set from Hindsight's native full-bank vector index. The response is approximate and does not claim an exhaustive match total.
 ...
 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param bankId
 @return ApiRetrieveSemanticCandidatesRequest
*/
func (a *MemoryAPIService) RetrieveSemanticCandidates(ctx context.Context, bankId string) ApiRetrieveSemanticCandidatesRequest {
	return ApiRetrieveSemanticCandidatesRequest{
		ApiService: a,
		ctx: ctx,
		bankId: bankId,
...
func (a *MemoryAPIService) RetrieveSemanticCandidatesExecute(r ApiRetrieveSemanticCandidatesRequest) (*SemanticCandidatesResponse, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *SemanticCandidatesResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "MemoryAPIService.RetrieveSemanticCandidates")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v1/default/banks/{bank_id}/memories/semantic-candidates"
	localVarPath = strings.Replace(localVarPath, "{"+"bank_id"+"}", url.PathEscape(parameterValueToString(r.bankId, "bankId")), -1)

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.semanticCandidatesRequest == nil {
		return localVarReturnValue, nil, reportError("semanticCandidatesRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.semanticCandidatesRequest
	req, err := a.client.prepareRequest(r.ctx, localVarPath, localVarHTTPMethod, localVarPostBody, localVarHeaderParams, localVarQueryParams, localVarFormParams, formFiles)
	if err != nil {
		return localVarReturnValue, nil, err
	}
