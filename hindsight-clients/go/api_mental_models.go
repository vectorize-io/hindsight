...
Create a mental model. If content is omitted, the model is created with a placeholder and reflect runs in the background to generate content; the response includes an operation_id for tracking. If content is provided, that content is stored directly and no refresh is scheduled at create time (operation_id is null). Authored content is not permanent: later refreshes still rewrite it according to trigger.mode — 'full' (default) regenerates from source_query and discards the authored text, while 'delta' edits the stored content in place. Pass trigger.mode='delta' to keep the injected content as the baseline for those edits. Use the update endpoint to replace content directly.
...
Update a mental model's name, source query, tags, trigger settings, and/or authored content. Providing content replaces the stored document directly without running reflect, and it is not permanent: later refreshes still rewrite it according to trigger.mode — 'full' (default) regenerates from source_query and discards the authored text, while 'delta' edits the stored content in place. Pass trigger.mode='delta' to keep the authored content as the baseline for those edits.
