...
export async function ingestGit(
  ...
  let failures = 0;
  await pool(shas, opts.concurrency ?? 8, async (sha) => {
    const [h, an, ae, aISO, cISO, subj, body] =
      git(repo, "show", "-s", `--format=%H${US}%an${US}%ae${US}%aI${US}%cI${US}%s${US}%b`, sha).split(US);
  ...
  }, (i, e) => { failures++; log(`  ! commit ${i} failed to enqueue: ${(e as Error).message?.slice(0, 120)}`); },
     (done) => { if (done % 25 === 0) log(`  ${done}/${shas.length}`); });
