...
| `retainDocumentScope`      | `"session"`                    | Granularity of retained document IDs. `"session"` appends turns to one document per OpenClaw session when the Hindsight API supports append. `"turn"` creates a separate document per retain (`...:turn:000001` / `...:window:000002`).                                                                     |
