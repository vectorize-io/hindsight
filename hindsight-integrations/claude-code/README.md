...
| `retainTags` | — | `["{session_id}", "surface:claude-code"]` | Tags attached to the retained document. The default `surface:claude-code` tag identifies which integration wrote a fact, so multi-surface setups sharing one bank can attribute and filter memories per surface (e.g. `recall` with `tags=["surface:claude-code"]`, or a `tag_groups` exclusion). Supports template placeholders: `{session_id}`, `{bank_id}`, `{timestamp}`, and `{user_id}` (resolved from `HINDSIGHT_USER_ID` env var; empty string if unset). Tags whose resolved form ends in an empty namespace part (e.g. `"user:"` when `HINDSIGHT_USER_ID` is unset) are dropped from the outgoing request. See [Template variables](#template-variables-for-retaintags-and-retainmetadata) below. |
