...
def detect_llm_config(config: dict) -> dict:
    ...
    if cfg_provider:
        ...
        # HINDSIGHT_API_LLM_API_KEY is documented as the highest-priority key
        # source, but this branch used to ignore it: a config naming llmProvider
        # with only that env var set was rejected as "no API key found", or worse
        # picked up a stale OPENAI_API_KEY meant for something else.
        if override_key:
        ...
        return {
            ...
            "api_key": api_key,
            # Env var first: this module documents HINDSIGHT_API_LLM_* as the
            # highest-priority source, and base_url below already honours that.
            # The other order let a configured llmModel silently beat an
            # explicit HINDSIGHT_API_LLM_MODEL, so the daemon ran a different
            # model than the environment asked for.
            "model": override_model or config.get("llmModel"),
            "base_url": override_base_url,
