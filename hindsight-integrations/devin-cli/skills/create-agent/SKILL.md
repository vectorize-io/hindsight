...
`<path>` must be inside `~/.self-driving-agents/`, at any depth. That is where
the installer writes, and the `allowed-tools` grant above is deliberately
scoped to it — this skill reads whatever it is pointed at and ingests it into
long-term memory, so a grant over arbitrary paths would make "create an agent"
a way to read any file on the machine. Nested subdirectories are fine. If the
user names a path outside that tree, say so and ask them to move or re-run the
installer rather than trying to read it.
...
Write to `~/.config/devin/agents/<name>/AGENT.md` (Devin CLI's [custom subagent](https://docs.devin.ai/cli/subagents#custom-subagents) format — a `name.md` file directly under `agents/` also works, but the `<name>/AGENT.md` form is preferred for consistency with plugin-shipped subagents):

Devin CLI subagent frontmatter has no `mcpServers` field (unlike Claude Code) — MCP servers are registered session-wide (by this plugin's `setup` skill), and any subagent without a restrictive `allowed-tools` list already has access to them. If the agent-specific description calls for restricting tools, include `mcp__hindsight__*` in `allowed-tools` alongside whatever else it needs.
...
- Don't add an `allowed-tools` restriction unless the user's description calls for one — that's what gives the subagent access to the `hindsight` MCP server's tools (registered session-wide by this plugin's `setup` skill). If you do restrict tools, include `mcp__hindsight__*`.
...
1. Confirm the subagent file was written to `~/.config/devin/agents/<name>/AGENT.md`
2. Tell the user they can invoke the agent by name via `run_subagent`, or that Devin will consider it alongside the built-in subagent profiles
3. If hooks/MCP aren't set up yet, point them at this plugin's `setup` skill first — the subagent's `mcpServers: [hindsight]` reference needs that MCP server registered to work
