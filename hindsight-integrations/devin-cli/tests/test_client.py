...
class TestApiUrlWithQueryOrFragmentIsRejected:
    """request() appends the endpoint path by concatenation.

    A query or fragment on the configured base URL swallows that path, so the
    request lands somewhere other than the intended route — silently, with a
    plausible-looking URL. Rejected rather than stripped: dropping a
    deliberately configured ?tenant= would be the worse failure.
    """

    @pytest.mark.parametrize(
        "url",
        [
            "https://host/api?tenant=x",
            "https://host/api#frag",
            "http://localhost:9077/?debug=1",
        ],
    )
    def test_query_or_fragment_is_rejected(self, url):
        with pytest.raises(ValueError, match="query or fragment"):
            _validate_api_url(url)

    def test_a_plain_url_with_a_path_is_still_accepted(self):
        assert _validate_api_url("https://host/api/") == "https://host/api"
...
class TestHindsightClientRecall:
    ...
    def test_sends_user_agent_header(self):
        ...
        assert captured["ua"].startswith("hindsight-devin-cli/")
...
class TestRequestTimeoutOverride:
    ...
    @pytest.mark.parametrize("bad", ["60", 0, -5, True, [], {}, "abc"])
    def test_an_unusable_override_falls_back_to_the_per_call_timeout(self, bad):
        """This setting escapes config.py's central type check — its default is None.

        A string reaches urlopen and raises TypeError inside a hook; a zero or
        negative value makes every request fail instantly. Both are worse than
        ignoring the setting.
        """
        c = HindsightClient("http://localhost:9077", request_timeout_override=bad)
        captured = {}
        ...
        assert captured["timeout"] == 10, f"unusable override {bad!r} was not ignored"
...
class TestPluginVersionNeverCrashesImport:
    """_plugin_version() runs at module import, so anything it raises is fatal.

    Every hook imports this client; an uncaught AttributeError here kills them
    all before a single request is made. `.get()` on valid-but-non-object JSON
    was exactly that — neither OSError nor ValueError, so it escaped.
    """

    @pytest.mark.parametrize("manifest", ["[]", '"a string"', "null", "42"])
    def test_non_object_manifest_falls_back(self, tmp_path, monkeypatch, manifest):
        plugin_dir = tmp_path / "plugin" / ".devin-plugin"
        plugin_dir.mkdir(parents=True)
        (plugin_dir / "plugin.json").write_text(manifest)
        monkeypatch.setattr(client_module, "__file__", str(tmp_path / "plugin" / "scripts" / "lib" / "client.py"))

        assert client_module._plugin_version() == "0.0.0"

    def test_non_string_version_falls_back(self, tmp_path, monkeypatch):
        plugin_dir = tmp_path / "plugin" / ".devin-plugin"
        plugin_dir.mkdir(parents=True)
        (plugin_dir / "plugin.json").write_text('{"version": 3}')
        monkeypatch.setattr(client_module, "__file__", str(tmp_path / "plugin" / "scripts" / "lib" / "client.py"))

        assert client_module._plugin_version() == "0.0.0"

    def test_a_real_manifest_is_read(self, tmp_path, monkeypatch):
        plugin_dir = tmp_path / "plugin" / ".devin-plugin"
        plugin_dir.mkdir(parents=True)
        (plugin_dir / "plugin.json").write_text('{"version": "1.2.3"}')
        monkeypatch.setattr(client_module, "__file__", str(tmp_path / "plugin" / "scripts" / "lib" / "client.py"))

        assert client_module._plugin_version() == "1.2.3"
