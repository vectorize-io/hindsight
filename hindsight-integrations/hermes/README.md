...
| `bank_id_by_user` | — | Optional JSON object mapping a platform-scoped gateway key (`<platform>:<user_id>`, e.g. `dingtalk:<id>`) to a bank id. Matching entries override the static/template bank; raw user-id keys are only used when no platform is available, preventing cross-platform ID collisions. |
