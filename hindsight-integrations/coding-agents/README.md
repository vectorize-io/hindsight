...
| `toolAllowlist`         | all                                  | exact `hindsight_*` tool suffixes exposed through MCP and native plugin adapters; `["recall"]` exposes only recall; set `autoReflect` to `false` to disable automatic reflection; server-side token scopes remain authoritative     |
