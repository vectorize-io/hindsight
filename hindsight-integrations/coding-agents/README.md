...
| `toolAllowlist`         | all                                  | exact `hindsight_*` tool suffixes exposed through MCP and native plugin adapters; `["recall"]` creates a recall-only tool surface, while automatic reflection remains controlled by `autoReflect`; server-side token scopes remain authoritative |
