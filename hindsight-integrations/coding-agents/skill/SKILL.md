...
| `chatBatch`             | `5`                                  | per-run cap on conversation history import: a background run ingests the N most recent not-yet-ingested sessions and later runs take the rest, pacing a long session history the way per-commit diffs already are (`0` = no cap). The attended `install --import-conversations` is exempt and still imports everything in one run                                                                                                                 |
