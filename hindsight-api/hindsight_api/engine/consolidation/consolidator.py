...
async def _trigger_mental_model_refreshes(
    ...
    # SECURITY: Only refresh models with overlapping tags (or untagged models, or if no tags were consolidated)
    async with pool.acquire() as conn:
        if consolidated_tags:
            # Filter by tag overlap: mental model tags must overlap with consolidated tags
            # OR mental model has no tags (untagged mental models are refreshed regardless)
            rows = await conn.fetch(
                ...
                WHERE bank_id = $1
                  ...
                  AND (tags IS NULL OR tags = '{{}}' OR tags && $2::varchar[])
                """,
                bank_id,
                consolidated_tags,
            )
        else:
            # No tags were consolidated (all untagged memories) - refresh all mental models
            rows = await conn.fetch(
                f"""
                SELECT id, name, tags
                FROM {fq_table("mental_models")}
