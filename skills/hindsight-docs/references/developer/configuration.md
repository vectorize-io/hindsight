...
| `HINDSIGHT_API_EMBEDDINGS_TEI_BATCH_SIZE` | Max texts per TEI `/embed` request. Throughput comes from concurrent requests rather than large ones, so the default is deliberately small and paired with `HINDSIGHT_API_EMBEDDINGS_MAX_CONCURRENT_REQUESTS`; TEI's own `--max-client-batch-size` (32 by default) is a hard validation error, so raising this above the server's value fails the request rather than being clamped | `8` |
