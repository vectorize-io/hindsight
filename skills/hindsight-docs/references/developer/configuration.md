...
| `HINDSIGHT_API_DEFAULT_MENTAL_MODEL_REFRESH_MODE` | Default refresh mode (`full` or `delta`) for mental models whose `trigger.mode` is not set explicitly. `full` regenerates the content from scratch on each refresh; `delta` performs a surgical edit against the existing content when a baseline exists (falling back to `full` when it doesn't). An explicit `trigger.mode` on a model always overrides this. Invalid values fall back to `full`. | `full` |
