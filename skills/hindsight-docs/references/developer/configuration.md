...
| `HINDSIGHT_API_CONSOLIDATION_MAX_ATTEMPTS` | Outer retry attempts for the consolidation LLM batch call. Each attempt uses the inner retry budget (`HINDSIGHT_API_CONSOLIDATION_LLM_MAX_RETRIES`). Language validation can add one corrective call, which uses the same inner retry budget. | `3` |
| `HINDSIGHT_API_CONSOLIDATION_LANGUAGE_VALIDATION` | When no explicit `HINDSIGHT_API_LLM_OUTPUT_LANGUAGE` is configured, validate CREATE/UPDATE observation text against the dominant language of its new source facts, retrying once with a corrective instruction when it drifts. | `true` |
| `HINDSIGHT_API_CONSOLIDATION_LANGUAGE_VALIDATION_FAILURE_POLICY` | Action after the corrective retry still produces a language mismatch: `fail_batch` leaves the source facts unpersisted for retry, while `fail_open` keeps the response and emits a warning. | `fail_batch` |
