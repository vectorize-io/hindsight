...
| `HINDSIGHT_API_CONSOLIDATION_POOLING_TAGS_MATCH` | Tag-match strategy for pooling existing observations as merge candidates during consolidation: `all_strict` or `any_strict`. `all_strict` requires a candidate to carry every tag of the fact being consolidated and is the security boundary that keeps consolidation scoped within a tag-defined tenant/user boundary. `any_strict` widens pooling to an OR-match, useful when a bank's tags are purely descriptive (e.g. classifier-assigned category/tool labels) rather than a scoping boundary. Configurable per bank. | `all_strict` |
