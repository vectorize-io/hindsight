...
This is controlled by the [`HINDSIGHT_API_CONSOLIDATION_DEDUP_THRESHOLD`](configuration.md#observations) setting: the cosine similarity at or above which two observations are reconciled. It is **disabled by default** (`1.0`) and is available on PostgreSQL deployments. Lower it (e.g. `0.97`) to enable reconciliation; a lower value reconciles more aggressively.
