...
Changing a bank's **embedding model** (e.g. a 384-dim encoder → a 1024-dim one), **vector extension** (pgvector / vchord / pgvectorscale), or **text-search backend** can't be done in place on a populated bank — the stored vectors and indexes are tied to those settings. Because every embedding and index is a deterministic function of text already on disk, the supported path is to **move the bank to a fresh instance configured with the new settings and re-derive everything there — no LLM re-extraction**.

This is a whole-bank superset of the document transfer above: it carries documents, facts, observations, bank config (disposition, mission, config), mental models, directives and webhooks — but **never embeddings**, which are regenerated by the target instance's model on import.

Two admin commands (PostgreSQL only):

```bash
# On the SOURCE instance — read-only, no model needed:
uv run hindsight-admin export-bank --bank my-bank --output my-bank.zip
# add --include-history to also carry audit_log / llm_requests (off by default)

# On the TARGET instance, configured with the new embedding/vector/text-search settings:
uv run hindsight-admin import-bank --archive my-bank.zip
```

`import-bank` re-embeds facts with the target instance's model, rebuilds links and indexes, and restores config/mental-models/directives/webhooks **exactly** (it does not fire webhooks or re-run consolidation — a migration restores state, it doesn't regenerate it).

### Blue-green runbook

1. Stand up a **new instance** pointed at a fresh database, configured with the new embedding model / vector extension / text-search backend.
2. Quiesce writes to the source bank (maintenance window) and run `hindsight-admin backup` for safety.
3. `export-bank` on the source → `import-bank` on the target.
...
> **📝 Why a new instance, not in-place**
> 
The embedding model is server-level, and a bank's `memory_units.embedding` column has a single dimension shared across the schema, so a different-dimension or different-backend bank needs its own instance/database. The old vectors are never mutated, which is what makes rollback trivial.
