        ...
        measure(
            ...
            lambda: sum(
                ...
                for _ in _iter_raw_sub_batches(
                    ...
                    # The generated document is text-only, so the image budget never
                    # binds; these are the server defaults, passed explicitly because
                    # the splitter requires every caller to state its chunking terms.
                    image_cost_chars=DEFAULT_RETAIN_IMAGE_CHUNK_COST_CHARS,
                    max_images_per_chunk=DEFAULT_RETAIN_MAX_IMAGES_PER_CHUNK,
                )
            ),
