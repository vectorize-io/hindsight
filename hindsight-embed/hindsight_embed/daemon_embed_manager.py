...
class DaemonEmbedManager(EmbedManager):
    ...
    def _receipt_owned_pid_on_port(
        self,
        port: int,
        pid_file: Path,
        markers: tuple[str, ...],
        ...
        """Return an identified listener only when its ownership receipt also matches."""
        listener_pid = self._owned_pid_on_port(port, markers, description)
    ...
    def _clear_port(self, port: int, pid_file: Path | None = None) -> bool:
        ...
        Behavior:
          ...
          * Port occupied but /health is unreachable, non-200, or does not return
            ...
            attempt to reclaim it only after the listener's command line identifies
            it as Hindsight and its PID-and-birth-marker receipt matches. This
            preserves stale-daemon recovery without SIGTERMing a foreign or
            PID-reused process (issue #3520).
          * Kill failed, or missing/mismatched ownership → False.
        """
        if not self._is_port_in_use(port):
            return True
        ...
        pid = self._receipt_owned_pid_on_port(
            port,
            pid_file,
