...
def parse_chunk_id(chunk_id: str | None) -> tuple[str, str, int] | None:
    ...
    try:
        ...
        return None
    return _unescape(bank_id), _unescape(document_id), index
...
def document_and_index_in(chunk_id: str, bank_id: str) -> tuple[str, int] | None:
    """Split ``chunk_id`` into ``(document_id, chunk_index)`` given the bank that owns it.
    ...
    for prefix, escaped in ((f"{_escape(bank_id)}_", True), (f"{bank_id}_", False)):
        ...
        if not document_id or not idx_s.isdigit():
            continue
        return (_unescape(document_id) if escaped else document_id), int(idx_s)
    return None
