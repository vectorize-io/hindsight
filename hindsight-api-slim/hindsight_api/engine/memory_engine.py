    ...
    async def update_mental_model(
        ...
        *,
        ...
        if content is not None:
            ...
            if embedding:
                new_embedding_vec = list(embedding[0])
        ...
        async with acquire_with_retry(backend) as conn:
            ...
            if content is not None:
                ...
                # Apply the embedding computed above (off-connection). Skipped once the store
                # owns the index — the vector still gets computed, because the store needs it, it
                # just stops being written to a column nothing reads for this bank.
                if new_embedding_str is not None and self._pg_carries_page_search(bank_id):
            ...
            # document (it is name + content), and re-embedding is what keeps the vector from going
            # stale behind it — which is why the embedding is recomputed here rather than reusing
            # `new_embedding_vec`, which is None for a name-only change.
            if row is not None and (name is not None or content is not None):
                vec = new_embedding_vec
                if vec is None and self._knowledge_index_store(bank_id) is not None:
                    vec = await self._mental_model_embedding_vector(row["name"] or "", row["content"] or "")
        ...
        async with acquire_with_retry(backend) as conn:
            async with conn.transaction():
                ...
                if row is not None and row["mental_model_id"] is not None:
                    ...
                    # The name is half the searchable document, so a rename changes what BM25
                    # matches AND what the embedding should be. Re-embedding here is why a renamed
                    # page does not drift out of vector reach behind its own title.
                    vec = None
                    if self._knowledge_index_store(bank_id) is not None:
                        mm = await conn.fetchrow(
                            f"SELECT name, content FROM {fq_table('mental_models')} WHERE bank_id = $1 AND id = $2",
                            bank_id,
                            row["mental_model_id"],
                        )
                        if mm is not None:
                            vec = await self._mental_model_embedding_vector(mm["name"] or "", mm["content"] or "")
                    await self._index_knowledge_page(conn, bank_id, row["mental_model_id"], embedding=vec)
