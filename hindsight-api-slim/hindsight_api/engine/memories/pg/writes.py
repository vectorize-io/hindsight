...
async def delete_stale_observations(
    *,
    ...
    # writes next and deadlock (#4251). Callers that delete or rewrite their facts first
    # therefore also sweep before that write, so their locks are taken in this order too.
    await conn.fetch(
