...
class LLMProvider:
    ...
    def __init__(
        ...
        Args:
            ...
            member_id: Stable opaque identifier for this member within a multi-LLM
                chain. Generated by ``_generate_member_id`` in ``config.py`` and
                stamped onto ``_provider_impl.member_id`` so ``batch_provider_impl``
                can resolve the exact account that submitted a batch after a crash
                recovery (same-provider disambiguation, #3649 follow-up). ``None``
                for single-provider paths and providers constructed directly (e.g. tests
                or ``from_env``), where the ``getattr`` fallback to ``name``/``provider``
                still applies.
        ...
        # Stamp the stable member identity onto the impl so batch_provider_impl()
        # and fact_extraction's crash-recovery guard can select the exact member
        # that submitted a batch (same-provider disambiguation, #3649 follow-up).
        self.member_id = member_id
        self._provider_impl.member_id = member_id
