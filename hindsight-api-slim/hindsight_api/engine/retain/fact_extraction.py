...
def _structured_chunk_size(max_chars: int, structured_chunk_size: int | None) -> int:
    return structured_chunk_size if structured_chunk_size is not None else max_chars
...
def chunk_text(text: str, max_chars: int, structured_chunk_size: int | None = None) -> list[str]:
    ...
    try:
        ...
        if isinstance(parsed, list) and all(isinstance(turn, dict) for turn in parsed):
            ...
            return _chunk_conversation(parsed, max_chars, structured_chunk_size)
    except (json.JSONDecodeError, ValueError):
        pass

    # Try to parse as JSONL (newline-delimited JSON objects, e.g. session logs)
    jsonl_chunks = _chunk_jsonl(text, max_chars, structured_chunk_size)
...
def _chunk_conversation(turns: list[dict], max_chars: int, structured_chunk_size: int | None = None) -> list[str]:
    """
    ...
    Args:
        ...
        structured_chunk_size: Maximum characters for a single turn to keep whole

    Returns:
        List of JSON-serialized chunks, each containing complete turns
    """

    structured_limit = _structured_chunk_size(max_chars, structured_chunk_size)
...
def _chunk_jsonl(text: str, max_chars: int, structured_chunk_size: int | None = None) -> list[str] | None:
    ...
    whole only up to ``structured_chunk_size``. When unset, that limit
    defaults to ``max_chars``. Returns ``None`` if the input is not JSONL, so the
    caller falls back to plain-text splitting.

    Args:
        text: Input text to inspect/chunk.
        max_chars: Maximum characters per chunk.
        structured_chunk_size: Maximum characters for a single JSONL line to
            keep whole.
