...
def _build_user_message(
    ...
    speaker_section = ""
    if agent_name:
        speaker_section = f"\nSpeaker: {agent_name} (AI agent) — first-person content (\"I\", \"we\") refers to this agent, not the user"

    return f"""Extract facts from the following text chunk.

Chunk: {chunk_index + 1}/{total_chunks}
Event Date: {event_date_str}
Context: {sanitized_context}{speaker_section}{metadata_section}
    ...
    for content_index, item in enumerate(contents):
        ...
        for chunk_index_in_content, chunk in enumerate(chunks):
            ...
            user_message = _build_user_message(
                chunk, chunk_index_in_content, len(chunks), item.event_date, item.context, item.metadata or None, agent_name
            )
