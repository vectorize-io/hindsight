...
# Set the extension context on every extension that needs system access: tenant extensions for
# schema provisioning, operation validators for anything their hooks read from the engine. The
# context is built once and shared, and only when an extension actually needs it —
# `Extension.context` raises RuntimeError rather than returning None, so an extension that was
# never given one cannot recover at runtime.
