...
class CreateMentalModelRequest(BaseModel):
    ...
    content: str | None = Field(
        default=None,
        description=(
            "Optional authored content to store directly. When provided, the mental model is created "
            "with this content and no refresh is scheduled at create time. When omitted, content is "
            "generated asynchronously from source_query via reflect. Later refreshes still rewrite the "
            "content according to trigger.mode; set trigger.mode='delta' to keep this content as the "
            "baseline those edits start from."
        ),
    )
...
class CreateMentalModelResponse(BaseModel):
    ...
    operation_id: str | None = Field(
        default=None,
        description=(
            "Operation ID to track refresh progress when content is generated asynchronously. "
            "Null when content was provided directly and no refresh was scheduled."
        ),
    )
...
class UpdateMentalModelRequest(BaseModel):
    ...
    content: str | None = Field(
        default=None,
        description=(
            "Optional authored content to store directly. When provided, replaces the current content "
            "without running reflect. Use the clear endpoint to empty content instead of passing an "
            "empty string."
        ),
    )
...
def _register_routes(app: FastAPI):
    ...
    @app.post(
        ...
        description=(
            "Create a mental model. If content is omitted, the model is created with a placeholder and "
            "reflect runs in the background to generate content; the response includes an operation_id "
            "for tracking. If content is provided, that content is stored directly and no refresh is "
            "scheduled at create time (operation_id is null). Authored content is not permanent: later "
            "refreshes still rewrite it according to trigger.mode — 'full' (default) regenerates from "
            "source_query and discards the authored text, while 'delta' edits the stored content in "
            "place. Pass trigger.mode='delta' to keep the injected content as the baseline for those "
            "edits. Use the update endpoint to replace content directly."
        ),
    ...
    async def api_create_mental_model(
        ...
        """Create a mental model, optionally with authored content."""
        try:
            provided_content = body.content
            ...
            mental_model = await app.state.memory.create_mental_model(
                ...
                content=provided_content if provided_content is not None else "",
            ...
            # Authored content must not be immediately overwritten by the create-time
            # refresh. Later explicit/automatic refreshes can still replace it.
            if provided_content is not None:
                return CreateMentalModelResponse(mental_model_id=mental_model["id"], operation_id=None)

            # Schedule a refresh to generate the actual content
    ...
    @app.patch(
        ...
        description=(
            "Update a mental model's name, source query, tags, trigger settings, and/or authored content. "
            "Providing content replaces the stored document directly without running reflect, and it is "
            "not permanent: later refreshes still rewrite it according to trigger.mode — 'full' "
            "(default) regenerates from source_query and discards the authored text, while 'delta' edits "
            "the stored content in place. Pass trigger.mode='delta' to keep the authored content as the "
