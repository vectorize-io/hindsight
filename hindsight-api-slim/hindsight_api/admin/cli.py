...
@app.command(name="run-store-migration")
def run_store_migration(
    migration_id: str = typer.Argument(
        ...,
        help="The store's id for the migration, e.g. 'bank-catalog'. Named, not numbered: an id the deployed store does not have fails here rather than being silently skipped.",
    ),
    schema: str | None = typer.Option(
        None,
        "--schema",
        "-s",
        help="Single schema to migrate. Omit for every discovered tenant schema.",
    ),
    timeout: float = typer.Option(
        900.0,
        "--timeout",
        help="Seconds to wait per tenant before giving up on it and moving to the next. The work is NOT cancelled — the store keeps draining it — so a timeout means 'stop watching', not 'stop migrating'.",
    ),
    poll_interval: float = typer.Option(5.0, "--poll-interval", help="Seconds between status polls."),
    trigger_only: bool = typer.Option(
        False,
        "--trigger-only",
        help="Enqueue and return without waiting. For a migration whose size makes watching pointless; check progress later with the same command.",
    ),
):
    """Apply a store-side migration to every tenant.

    The counterpart of ``run-db-migration`` for a store that keeps derived state of its own. SQL
    migrations reshape tables; this brings the store's own data up to what a new build expects —
    the case neither alembic nor a normal write path can reach, because a bank nobody writes to is
    never revisited.

    Safe to re-run. Every store migration is required to be idempotent, so a partial run is
    recovered by running it again, and there is no state here to repair.

    This command does not record what it has done: alembic does, from the revision that calls it.
    """
    config = HindsightConfig.from_env()
    if not config.database_url:
        typer.echo("Error: Database URL not configured.", err=True)
    ...
    if failures:
        typer.echo(f"\n{len(failures)} tenant(s) did not converge: {', '.join(failures)}", err=True)
        raise typer.Exit(1)


async def _run_store_migration(
    migration_id: str,
    *,
    schema: str | None,
    ...
    """Trigger and watch `migration_id` per tenant. Returns the schemas that did not converge."""
    import time
    ...
    for name in schemas:
        # Per tenant, because the store resolves the tenant from the schema in context — the same
        # way every other store call in this process does.
        _current_schema.set(name)
        try:
            enqueued, namespaces = await store.run_store_migration(migration_id=migration_id)
        except Exception as e:  # noqa: BLE001 - one tenant must not abort the sweep
            typer.echo(f"  {name}: trigger failed: {e}", err=True)
        ...
        if namespaces == 0:
            ...
            continue
        typer.echo(f"  {name}: {enqueued} of {namespaces} namespace(s) enqueued")
        if trigger_only:
            continue

        deadline = time.monotonic() + timeout
        while True:
            remaining, stuck = await store.store_migration_status(migration_id=migration_id)
            if remaining == 0:
                typer.echo(f"  {name}: done")
                break
            if stuck and stuck >= remaining:
                # Everything left has been retried without progress. Waiting longer cannot help,
                # and reporting it now is the difference between a failed run and a hung one.
                typer.echo(f"  {name}: {remaining} namespace(s) stuck, not converging", err=True)
                failures.append(name)
                break
            if time.monotonic() > deadline:
                # Deliberately not a cancellation: the store keeps draining, so this only stops us
                # watching. Re-running the command later picks the same tenant up where it is.
                typer.echo(
                    f"  {name}: still {remaining} outstanding after {timeout:.0f}s; moving on "
                    f"(the store keeps working — re-run to check)",
                    err=True,
                )
                failures.append(name)
                break
