...
def test_minimax_factory_function():
    """Test that the create_llm_provider factory function creates MiniMax provider correctly."""
...
def test_minimax_default_model_config():
    """Test that MiniMax has MiniMax-M2.7 as default model in PROVIDER_DEFAULT_MODELS."""
    from hindsight_api.config import PROVIDER_DEFAULT_MODELS

    assert "minimax" in PROVIDER_DEFAULT_MODELS
    assert PROVIDER_DEFAULT_MODELS["minimax"] == "MiniMax-M2.7"
...
def test_minimax_m25_backward_compat():
    """Test that MiniMax-M2.5 model name still works (backward compatibility)."""
    llm = LLMProvider(
        provider="minimax",
        api_key="test-key",
        base_url="",
