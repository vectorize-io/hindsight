...
Here's a complete, runnable script. Save it as `research_crew.py`:
...
reflect_tool = HindsightReflectTool(
    bank_id=BANK_ID,
    budget="mid",
    reflect_context="You are helping a development team evaluate technologies.",
)

storage = HindsightStorage(
    bank_id=BANK_ID,
    mission="Track research findings, technical comparisons, and recommendations.",
)
...
researcher = Agent(
    ...
    goal="Find accurate, detailed information on the given topic.",
    backstory=(
        "You are a thorough technical researcher. Before starting new research, "
        "use the hindsight_reflect tool to check what you already know about the topic."
    ),
    tools=[reflect_tool],
    llm="openai/gpt-4o-mini",
)
...
topic = sys.argv[1] if len(sys.argv) > 1 else "the benefits of Rust for CLI tools"

research_task = Task(
    description=f"Research: {topic}",
    expected_output="A detailed summary with key findings and supporting evidence.",
    agent=researcher,
)

write_task = Task(
    description="Write a short technical article based on the research findings.",
    expected_output="A polished 3-paragraph article suitable for a developer blog.",
    agent=writer,
)
...
python research_crew.py "the benefits of Rust for CLI tools"
python research_crew.py "how Go compares to Rust for CLI tools"
python research_crew.py "which language should I pick for a new CLI tool"
```

Each run builds on the last. The third run has the full context of both prior research sessions.
...
- Crews that run repeatedly and need to compound knowledge
- Multi-agent workflows where past context improves future decisions
- Production systems where memory must survive restarts

**When not to use this:**

- One-shot crews that never run again
- Crews where you want each run to be completely independent
- Simple tasks where the overhead of retain/recall isn't justified

**Alternatives:**

- **CrewAI's built-in RAG memory**: Works within a single process. No persistence across restarts by default.
- **Custom SQLite/file storage**: You manage the schema, embeddings, and retrieval yourself.
- **Mem0**: Another external memory option CrewAI supports. Hindsight differs by offering multi-strategy retrieval (semantic + graph + temporal), structured fact extraction, and a synthesis engine (reflect).
