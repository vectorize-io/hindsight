...
description: Dry-run fact extraction, document OCR, sharper retrieval, richer MCP tool metadata and operational gauges, plus a retain data-integrity fix in Hindsight 0.8.3
...
- [**Richer MCP Tools**](#richer-mcp-tools): Read-only/destructive hints and restored custom tool instructions.
...
MCP clients get more to work with in 0.8.3:

- **Tool annotations.** MCP tools now carry read-only and destructive hints, so clients can present and guard them appropriately — for example, treating recall as read-only and curation actions as destructive.
- **Custom tool instructions restored.** Custom instructions attached to MCP tools are applied correctly again.
- **Cleaner reflect output.** Reflect responses no longer include tool/LLM traces by default, and `update_memory`/`invalidate_memory` ship with proper descriptions.
