...
description: Reranking with TypeSafe Jev, which ranks the whole recall pool in one question, move a bank anywhere with clone, export and rename, filter memories and documents by time on the server, a separate LLM for mental-model refreshes, and a round of tenant-isolation, deadlock and provider fixes
...
Hindsight 0.10.1 adds TypeSafe Jev as a reranker, which judges every recall candidate in a single question instead of one call each, and makes a bank something you can move: clone it in one call, export and import it with a defined scope, or rename it in place with its stored files following along. It also adds server-side time filtering to the memories and documents APIs, lets automatic mental-model refreshes run on their own LLM, and fixes a set of isolation, deadlock and provider bugs. **Everyone on 0.10.x should upgrade** — see the [changelog](/changelog) for the full list of fixes.

<!-- truncate -->

- [**Reranking with TypeSafe Jev**](#reranking-with-typesafe-jev-the-whole-pool-in-one-question): The new `typesafe` provider ranks every recall candidate in one question, and can optionally cut the irrelevant tail.
...
## Reranking with TypeSafe Jev: the whole pool in one question
