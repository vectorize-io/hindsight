...
- [**Browse Memory by Entity and Tag**](#browse-memory-by-entity-and-tag): An entity timeline, entity filters, and a reworked documents table with tag filtering.
...
- **A documents table built for scanning.** The documents list used to offer only a document-ID search. It now has a **tag filter** with autocomplete and an any/all toggle, and the table itself was reorganized around what you actually scan for: the last-updated time sits under the document ID, tags and metadata share a column, and size and memory-unit counts get their own aligned columns. Long document IDs no longer push everything else off the row.
