...
description: Optional document-text storage for data minimization, a vector-index fix for external PostgreSQL, a maintenance-routine fix for the public schema, and new integrations in Hindsight 0.8.1
...
Hindsight 0.8.1 is a patch release on top of [0.8.0](/blog/version-0-8-0): a new option to **skip storing raw document text** for data-minimization and privacy-sensitive deployments, two operational fixes for self-managed PostgreSQL, and a batch of new integrations. **Anyone running an external PostgreSQL with VectorChord, or a single-schema deployment, should upgrade.**
...
- [**New Integrations**](#new-integrations): Cursor CLI, Cline, Haystack, Obsidian, oh-my-openagent, and Roo Code.
...
0.8.1 lands a wave of new ways to connect Hindsight memory:

- **Cursor CLI** — capture and reuse Hindsight memory from Cursor CLI workflows.
- **Cline** — hooks into Cline's lifecycle events to save and recall memory automatically.
- **Haystack** — use Hindsight as a memory component in Haystack pipelines.
- **Obsidian** — an official plugin to sync and use Hindsight memory from your Obsidian vault.
- **oh-my-openagent (OMO)** — connect Hindsight memory to OMO workflows.
- **Roo Code** — now packaged as an installable PyPI CLI (`hindsight-roo-code`).
