...
- [**Clear Mental Model**](#clear-mental-model): Reset a mental model and force a full re-synthesis.
...
A new endpoint resets a mental model's content to empty, so the next refresh performs a full re-synthesis from scratch — regardless of whether the model is in full or delta mode:

```bash
POST /v1/default/banks/{bank_id}/mental-models/{id}/clear
```

This is useful for periodic compaction of delta-mode models that have drifted over many incremental refreshes. The Control Plane exposes it as a button in the mental-model detail view, and SDK examples are available in [the docs](/developer/api/mental-models).
