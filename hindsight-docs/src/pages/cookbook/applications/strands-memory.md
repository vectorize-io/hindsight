...
# Strands Agents + Hindsight Memory


:::info Complete Application
This is a complete, runnable application demonstrating Hindsight integration.
[**View source on GitHub →**](https://github.com/vectorize-io/hindsight-cookbook/tree/main/applications/strands-memory)
:::


Give your Strands agents persistent long-term memory. Chat with an assistant multiple times and watch it remember what you told it in previous sessions.
...
2. **OpenAI API key** (for Strands' LLM)
...
3. **Install dependencies**
   ...
   ```bash
   cd applications/strands-memory
...
`create_hindsight_tools()` returns `@tool`-decorated functions the agent can call:
...
### 3. Pre-Inject Memories into System Prompt

`memory_instructions()` performs a recall and returns a formatted string for injection into the system prompt:

```python
from hindsight_strands import memory_instructions
...
Any [Strands model provider](https://strandsagents.com/latest/user-guide/concepts/model-providers/) works:
...
**"No module named 'hindsight_strands'"**
```bash
pip install -r requirements.txt
```
...
**Built with:**
- [Strands Agents](https://strandsagents.com) - AWS open-source Python agent framework
- [hindsight-strands](https://github.com/vectorize-io/hindsight/tree/main/hindsight-integrations/strands) - Hindsight memory tools for Strands
