...
| `HINDSIGHT_API_LLM_CACHE_AFFINITY` | Pin a conversation to one backend prompt cache on the OpenAI-compatible providers, Fireworks and Nous. Server-side prompt caches are per backend server, so the same conversation has to reach the same one for a cache hit. `xai_conv_id` sends xAI's documented `x-grok-conv-id` header; `header` sends the operation ID under the configured `HINDSIGHT_API_LLM_CACHE_AFFINITY_HEADER` name; `openai_prompt_cache_key` sends OpenAI's `prompt_cache_key` field; `auto` picks one from the configured base URL host (x.ai / grok.com &rarr; `xai_conv_id`; native OpenAI or an openai.com host &rarr; `openai_prompt_cache_key`; anything else, including Azure OpenAI, &rarr; `none`); `none` sends nothing. The id is the operation's trace id, so every LLM call of one retain/reflect/consolidation run shares it. `header` is explicit-only and is never selected by `auto`. `auto` is an allowlist rather than a best-effort probe, so an unknown OpenAI-compatible backend (vLLM, ollama, groq, openrouter, a custom proxy) receives byte-identical requests to before; set `none` to disable entirely. Anthropic and Gemini/Vertex do their own caching and ignore this. | `auto` |
| `HINDSIGHT_API_LLM_CACHE_AFFINITY_HEADER` | HTTP header name required when affinity is `header` (for example, `x-session-id` **only if your backend documents support for it**). No gateway support is implied. Sends a stable 32-hex operation ID. Operator values in SDK `default_headers` or per-call headers win case-insensitively in this mode. Existing `xai_conv_id` and required OpenCode session-header behavior are unchanged. Invalid or missing names fail provider construction in header mode. Indexed chain members may set `HINDSIGHT_API_<OP>LLM_<n>_CACHE_AFFINITY_HEADER`; unset members inherit the global name. | Unset |
...
| `HINDSIGHT_API_RETAIN_LLM_CACHE_AFFINITY_HEADER` | Header name for retain header-mode affinity; resolved independently of the mode | Falls back to `HINDSIGHT_API_LLM_CACHE_AFFINITY_HEADER` |
...
| `HINDSIGHT_API_REFLECT_LLM_CACHE_AFFINITY_HEADER` | Header name for reflect header-mode affinity; resolved independently of the mode | Falls back to `HINDSIGHT_API_LLM_CACHE_AFFINITY_HEADER` |
