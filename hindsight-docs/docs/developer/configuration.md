...
| `HINDSIGHT_API_CONSOLIDATION_WINDOW_START` | Restrict the consolidation pipeline to a daily off-peak window, expressed as a wall-clock time in the window timezone (`HH:MM`, 24h). Tasks that land outside the window are deferred (held as `pending` with `next_retry_at` set to the next window open) instead of failing the LLM-heavy work during peak hours. Set both `START` and `END` to enable; leave both unset for always-on (the default). `START > END` means a cross-midnight window (e.g. `22:00`–`06:00`); `START == END` is treated as always-open. Server-level (static). | unset |
| `HINDSIGHT_API_CONSOLIDATION_WINDOW_END` | Window end (`HH:MM`). Half-open interval `[start, end)` — the instant exactly at `END` is outside. Must be set together with `START`. | unset |
| `HINDSIGHT_API_CONSOLIDATION_WINDOW_TZ` | IANA timezone name the window boundaries are evaluated in (e.g. `Asia/Shanghai`, `America/New_York`). DST is handled per the zone rules. Unknown names fall back to `UTC` with a warning. | `UTC` |
