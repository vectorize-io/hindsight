...
| `HINDSIGHT_API_WORKER_ORPHAN_LEASE_SECONDS` | Seconds a `processing` operation may sit untouched before **another** worker may reclaim it. Covers the case where `worker_id` falls back to the container hostname: each redeploy mints a new id, so work in flight at that moment is stranded under an id that never returns and no later startup can match it. Liveness is judged on `claimed_at` **and** the progress heartbeat, so a slow-but-live job is never taken. A reclaimed row spends a retry and is failed at the cap, so a task that kills each worker it touches cannot loop forever. `0` disables. Pinning `HINDSIGHT_API_WORKER_ID` is the real fix; this is the safety net. | `86400` |
