...
| `HINDSIGHT_API_RECALL_DEDUP_THRESHOLD` | Collapses near-duplicate raw `world`/`experience` recall results after ranking and before token filtering. `1.0` only collapses exact normalized duplicates; lower values are fuzzier. `0` disables the filter. | `1.0` |
