...
| `HINDSIGHT_API_ENTITY_LLM_ARBITRATION` | Gray-band LLM arbitration for entity resolution: mentions whose blended score lands just under the acceptance threshold (or low-entropy names with plausible candidates) are decided by one small-LLM call over the co-occurrence context instead of always creating a new entity. The model can only pick an existing candidate or none — it never invents entities or merges existing rows. Adds LLM cost per affected retain batch. Configurable per bank. | `false` |
| `HINDSIGHT_API_ENTITY_GRAY_BAND` | Lower bound of the blended-score band deferred to arbitration (the upper bound is the 0.6 acceptance threshold). Configurable per bank. | `0.45` |
