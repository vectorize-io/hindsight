...
| `HINDSIGHT_API_CONSOLIDATION_LLM_PARALLELISM` | Number of tag groups consolidated concurrently within one consolidation op. Batches within a tag group still run serially (they share an observation scope); distinct tag groups run in parallel. `1` = sequential (default). Keep at `1` for banks using `per_tag` / `all_combinations` observation scopes. Configurable per bank. | `1` |
