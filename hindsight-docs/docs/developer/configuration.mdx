...
| `HINDSIGHT_API_PG0_MAX_CONNECTIONS` | Embedded PostgreSQL connection limit. Server-level only; requires restarting the embedded database. Does not change external PostgreSQL servers. | `300` |
...
For embedded PostgreSQL, keep `PG0_MAX_CONNECTIONS` above the sum of the maximum sizes of all pools connecting to the same instance (including separate API/worker processes and any read pool targeting that instance), with spare connections for migrations, index maintenance, and administration. The default of `300` leaves headroom above the default primary pool of `100`; larger deployments must size both limits together. This setting applies when the embedded database starts, so restarting only an API process that attaches to an already-running instance does not apply a new limit.
