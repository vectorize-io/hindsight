...
By default, the dedup probe uses strict tag matching so it only compares observations inside the same tag scope. If you run a single-user/single-bank deployment and use unique per-session tags for provenance, set [`HINDSIGHT_API_CONSOLIDATION_DEDUP_TAG_MATCH`](/developer/configuration#observations) to `any` to let the dedup probe find near-duplicates across session tags. The consolidation recall that supplies LLM context still uses strict scope matching.
