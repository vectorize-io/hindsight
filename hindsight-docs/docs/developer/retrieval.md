...
The same precision is used when building embedding and BM25 date enrichment for newly retained, transferred, or curated memories: a year contributes `2026` and a month contributes `August 2026`, without an invented day. Exact-date formatting otherwise stays compatible with each index's existing renderer: BM25 includes the day, while embedding keeps its existing readable-date representation. Existing rows are not bulk re-embedded or reindexed by this change; their recall scoring and reranker context remain precision-aware, while operators who need refreshed candidate-generation signals can re-retain or transfer those memories.
