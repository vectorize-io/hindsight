...
| `toolAllowlist`         | all                                  | exact `hindsight_*` tool suffixes exposed through MCP and native plugin adapters; use `["recall"]` for a recall-only surface; server-side token scopes remain authoritative                                                       |
