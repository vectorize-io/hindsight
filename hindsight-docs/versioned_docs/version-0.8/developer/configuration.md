...
| `HINDSIGHT_API_CONSOLIDATION_DEDUP_TAG_MATCH` | Tag matching mode used only by the near-duplicate dedup probe (`any`, `all`, `any_strict`, or `all_strict`). The default keeps dedup within the same strict tag scope. Single-user/single-bank deployments that use unique per-session tags for provenance can set this to `any` so the probe can find near-duplicate observations across sessions. Consolidation recall still uses strict scope matching for LLM context. Configurable per bank. | `all_strict` |
