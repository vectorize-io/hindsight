...
/// interactive terminal. It is suppressed when stdout is piped or redirected,
/// when `NO_COLOR` is set (https://no-color.org), or when `TERM=dumb`.
/// Hindsight's primary consumers are coding agents that read piped output, so
/// escape codes there are noise at best and corrupt captured output at worst.
...
fn ansi_enabled_from(is_tty: bool, no_color: bool, term_dumb: bool) -> bool {
    is_tty && !no_color && !term_dumb
}
...
/// Logo for clap's `before_help`. The art is raw ANSI, so it is suppressed
/// when stdout is not a styled terminal (e.g. `hindsight --help | cat`).
...
mod tests {
    ...
    #[test]
    fn test_ansi_enabled_interactive_tty() {
        // Interactive terminal, no suppression requested -> styling on.
        assert!(ansi_enabled_from(true, false, false));
    }

    #[test]
    fn test_ansi_enabled_non_tty() {
        // Piped/redirected output -> styling off, regardless of env.
        assert!(!ansi_enabled_from(false, false, false));
        assert!(!ansi_enabled_from(false, true, true));
    }

    #[test]
    fn test_ansi_enabled_no_color() {
        // NO_COLOR set -> styling off even on a terminal.
        assert!(!ansi_enabled_from(true, true, false));
    }

    #[test]
    fn test_ansi_enabled_term_dumb() {
        // TERM=dumb -> styling off even on a terminal.
        assert!(!ansi_enabled_from(true, false, true));
    }
